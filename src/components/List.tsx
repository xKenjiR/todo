    ...
    <ul className="flex flex-col gap-2 mt-3">
      {list.map((item) =>
        item.complete ? (
          ...
          >
            ...
            <div
              className="delete font-medium text-red-600 hover:text-red-400 dark:text-red-500 dark:hover:text-red-600 transition-colors underline"
              onClick={(e) => {
                e.stopPropagation();
                deleteItem(item);
              }}
      ...
      {list.map((item) =>
        item.complete ? (
          <li
            key={item.id}
            className="todo flex justify-between p-3 rounded-md bg-gray-200 hover:bg-gray-300 dark:bg-gray-500 dark:hover:bg-gray-600 dark:text-white transition-colors shadow-md cursor-pointer"
            onClick={() => complete(item)}
          >
